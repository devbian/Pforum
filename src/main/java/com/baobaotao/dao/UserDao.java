...
public class UserDao extends BaseDao<User> {
	...
	public User getUserByUserName(String userName){
		List<Object> users = getHibernateTemplate().find(GET_USER_BY_USERNAME, userName);
	    if (users.size() == 0) {
			return null;
		}else{
	...
	public List<User> queryUserByUserName(String userName){
		List<Object> objects = getHibernateTemplate().find(QUERY_USER_BY_USERNAME,userName+"%");
